...
`firebase_admin` is the single import site for the SDK; the default app is
initialised once per process from `FIREBASE_CREDENTIALS_FILE`.
"""
...
from recally.models import Device, PushRun
...
def send_due_push(container: "Container", *, now: datetime | None = None) -> None:
    ...
    device. `now` is the naive-UTC clock the whole tick runs on (the same injection
    pattern as `list_due(now=...)`); the APScheduler and jobs.py callers leave it
    to wall time."""
...
def _send(device: Device, body: str, due_count: int, credentials_file: Path) -> bool:
    """One high-priority data message via FCM HTTP v1. True when FCM accepted it."""
