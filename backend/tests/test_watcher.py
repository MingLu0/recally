...
def test_burst_of_rename_events_debounces_to_one_ingest(tmp_path: Path) -> None:
    ...
    ingestor = RecordingIngestor()
    watcher = ExportWatcher(ingestor, watch_dir=tmp_path, debounce_ms=150)
    ...
    assert ingestor.called.wait(timeout=2)
    assert ingestor.files == [destination.resolve()]


def test_move_out_of_watched_folder_is_ignored(tmp_path: Path) -> None:
    watch_dir = tmp_path / "watch"
    watch_dir.mkdir()
    source = watch_dir / "oreilly-annotations.csv"
    destination = tmp_path / "oreilly-annotations.csv"
    source.write_text("completed export")
    source.rename(destination)
    ingestor = RecordingIngestor()
    watcher = ExportWatcher(ingestor, watch_dir=watch_dir, debounce_ms=10)

    _MovedExportHandler(watcher).on_moved(FileMovedEvent(str(source), str(destination)))

    assert not ingestor.called.wait(timeout=0.5)
    assert ingestor.files == []
