...
| Post-approval edits | Text is editable; FSRS state survives the edit | Fixing a typo is a correction to the same retrieval task. Resetting the schedule would punish the fix and discard months of history, so cards would go unfixed |
| Deferring a card | Bury (today) and suspend (indefinite), both leaving FSRS state alone | The alternative is a dishonest rating, which corrupts the training data for both the FSRS optimizer and the Learner |
