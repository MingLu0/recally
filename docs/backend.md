...
Inside the runner that job is split further, because units resolve concurrently (ADR-015). **Resolution is session-free**: `_resolve_card` and `_resolve_unit_cards` take no `session`, hold no database handle, and return `_CardOutcome` values. **Persistence is main-thread-only**: `_persist_unit` is the one function on a unit's path that touches the session — writing the cards, flipping `processed`, accumulating the run counters and committing — and it runs on the main thread, one unit at a time, while workers resolve the rest. SQLAlchemy's `Session` is not thread-safe and its failure mode is silent, so the split is enforced at the signature level by `test_resolution_helpers_take_no_session` rather than by convention. `LLM_CONCURRENCY` (default 1, the sequential path) sets the pool width; units commit as they complete, so a failed unit leaves its siblings' work intact and its own highlights retryable.
