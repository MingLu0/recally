...
**Jobs**:
- **Filter**: drop low-value highlights — bare headings with no sibling context, navigation text, isolated short phrases. Real data (one export, 695 rows, 324 of them for *30 Agents Every AI Engineer Must Build*): ~4% of rows are under 40 characters, none of them figure/table references. Junk is a small minority, so the Curator should default to `keep`. Value filtering is separate from **cardability**: a fragment that can support no self-contained card — neither alone nor grouped — is dropped even when the words look valuable. The common case is a highlight that starts mid-sentence or with a dangling pronoun whose subject appears nowhere in the batch; kept, it forces the Writer to invent a subject (#253: such sources were ~3× over-represented in rejections).
- **Group**: fold sibling highlights that only make sense together into one curated unit. The illustrative case is a run of headings highlighted in sequence ("Stage 1: Task assignment", "Stage 2: Code synthesis", "Stage 3: Test synthesis") becoming one structural card; that run is not in the export we have, so a real grouping example still needs to be identified before the step 2 gate (see [roadmap.md](roadmap.md), step 1). Two more shapes are requirements, not options (#253 widened the job after production showed 83% of keep units wrapping exactly one highlight): a highlight that starts mid-sentence or with a dangling pronoun is folded into the adjacent highlight that supplies its subject, and a truncated highlight is paired with its fuller sibling when one exists in the batch. Grouping changes which unit a highlight belongs to, never its text — a clipped row stays clipped (hard rule 7). Grouping is semantic, within the same chapter and day; there is no positional key in the export beyond that ordering.
...
- Flag leech cards (failed 3+ times): propose rewrite as alternative explanation/analogy, or cross-link to a related highlight from another book. Rewrites go through the normal Writer ⇄ Critic → human approval path as new cards; the leech keeps its FSRS state until the human approves the rewrite, at which point the old card is set to `rejected` with `status_reason="superseded by <id>"`. A leech is counted deterministically as an approved card with 3 or more lifetime `review_logs` rows at `rating=1` (Again) — `review_logs` has no lapse counter and `reps`/`lapses` are derived from it (data-model.md), so the threshold is defined here.
...
Prompt and loop changes are measured by the replay harness (`evals/harness.py`, #243): it replays the Writer ⇄ Critic loop over the committed 45-unit eval set (#242) and scores agreement, false-keep, round-1 accept and rework calls against the human's recorded verdicts — never against the Critic's opinion, which is all live-run metrics measure. The produced card text is also read side by side against the previous run by a human, because no score catches cards going vaguer. Curator changes are invisible to that harness by design (the Curator never runs in it), so they are measured by the re-curation replay (`evals/curator_replay.py`, #253): it re-curates a database copy's highlights and compares structural metrics — single-highlight rate, mid-sentence and dangling-pronoun starts, truncated highlights left alone, average unit length — between the existing units and the re-curated ones.
