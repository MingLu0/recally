...
**Checks**: first, before any card is judged, whether the source can support a card worth making at all. The suspicion triggers are sentence fragments whose subject was never exported, chapter transitions and other connective prose, and claims so generic they are true of any system (#251; the measured standard is the human's recorded reject reasons: "too trivial", "too abstract", "too isolated", "does not make much sense") — but suspicion is not a verdict: a suspect source that still names something specific (a concrete list, a named pair, a mechanism, a claim a "why" card can be built on) can support a card, and its cards are judged per card on atomicity, unambiguity, self-containedness, non-triviality, factual fidelity to the highlight. Only a source with no nameable fact fails the first check outright.
...
- `revise` → that card only goes back to the Writer with the critique; sibling cards already accepted are kept as they are. After 3 rounds without `accept` → `status=needs_human`. `generation_rounds` is per card. A card on a suspect-but-specific source that guesses at missing context lands here — the rewrite asks only about what the surviving text says.
- `reject` (unsalvageable: the source itself carries nothing worth a card — a bare heading, a navigation line, a pure transition, a fragment or claim with no nameable fact in it) → `status=needs_human` with the critique in `status_reason`. `reject` is for every card from such a source, never `revise` — no rewrite saves a source that carries nothing. The pipeline never sets `rejected`; only a human does.
