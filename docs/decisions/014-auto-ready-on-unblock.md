...
7. **The mailbox is a safety net, not a channel.** The Run id is persisted in the state file and reused across restarts, and each tick drains the mailbox: heartbeats are dropped, everything else becomes an event line, and `ask` messages also fire a notification. Workers are instructed (in `orca-autostart-prompt.md`) never to use `orchestration ask` — human contact is a GitHub issue comment. The drain exists for workers that ignore the contract; the `agentWait` detector already catches parked workers independently.
