...
1. **A stateful Scala driver** (`scripts/orchestrate.sc`, scala-cli + cats-effect) automates dispatch, monitoring, failover and conflict recovery:
   ...
   - Rate-limit failures park the worker and retry the same task hourly until the usage window resets (single-agent pool). The pool, models, effort levels, cap, intervals and retry bounds are config in `orchestrator.json` (repo root; every key optional, defaults match this ADR); a `--agent=<id>[:model[:effort]]` flag overrides the pool for one run. Other failures retry once, then the issue is marked needs-human with a comment.
