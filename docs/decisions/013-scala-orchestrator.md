...
1. **A stateful Scala driver** (`scripts/orchestrate.sc`, scala-cli + cats-effect) automates dispatch, monitoring, failover and conflict recovery:
   ...
   - Rate-limit failures park the worker and retry the same task hourly until the usage window resets (single-agent pool; the pool order is config in `orchestrate.sc`). Other failures retry once, then the issue is marked needs-human with a comment.
